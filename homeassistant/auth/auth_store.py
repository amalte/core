...
class AuthStore:
    ...
    def _parse_data(self, data: dict) -> None:
        ...
        self._ensure_system_groups(
            groups, has_admin_group, has_user_group, has_read_only_group
        )

        self._parse_users(
            data, users, groups, group_without_policy, migrate_users_to_admin_group
        )

        self._parse_credentials(data, credentials, users)

        self._parse_refresh_tokens(data, users, credentials)
    ...
    def _parse_credentials(self, data: dict, credentials: dict, users: dict) -> None:
    ...
    def _parse_refresh_tokens(self, data: dict, users: dict, credentials: dict) -> None:
        for rt_dict in data["refresh_tokens"]:
            ...
            last_used_at = (
                dt_util.parse_datetime(rt_dict.get("last_used_at"))
                if rt_dict.get("last_used_at")
                else None
            )
