...
class AuthStore:
    ...
    async def async_load(self) -> None:  # noqa: C901
        ...
        self._load_users(data, perm_lookup)
        self._load_credentials(data)
        self._load_refresh_tokens(data)
        ...
        def _load_groups(self, data: dict) -> None:
            ...
            for group_dict in data.get("groups", []):
                name, policy, system_generated = self._parse_group(group_dict)
                ...
                has_admin_group = has_admin_group or group_dict["id"] == GROUP_ID_ADMIN
                has_user_group = has_user_group or group_dict["id"] == GROUP_ID_USER
                has_read_only_group = has_read_only_group or group_dict["id"] == GROUP_ID_READ_ONLY
                    
            self._add_system_groups(groups, has_admin_group, has_user_group, has_read_only_group)
            self._groups = groups    
            
        def _parse_group(self, group_dict: dict) -> tuple:
            """Parse group details from the group dictionary."""
            if group_dict["id"] == GROUP_ID_ADMIN:
                return GROUP_NAME_ADMIN, system_policies.ADMIN_POLICY, True
            if group_dict["id"] == GROUP_ID_USER:
                return GROUP_NAME_USER, system_policies.USER_POLICY, True
            if group_dict["id"] == GROUP_ID_READ_ONLY:
                return GROUP_NAME_READ_ONLY, system_policies.READ_ONLY_POLICY, True
            return group_dict["name"], group_dict.get("policy"), False

        def _add_system_groups(self, groups: dict, has_admin_group: bool, has_user_group: bool, has_read_only_group: bool) -> None:
            """Add system groups if they do not exist."""
            if not has_admin_group:
                groups[_system_admin_group().id] = _system_admin_group()
            if not has_user_group:
                groups[_system_user_group().id] = _system_user_group()
            if not has_read_only_group:
                groups[_system_read_only_group().id] = _system_read_only_group()

        def _load_users(self, data: dict, perm_lookup: PermissionLookup) -> None:
            """Load users from data."""
            users = {}
            migrate_users_to_admin_group = not self._groups

            for user_dict in data["users"]:
                user_groups = self._collect_user_groups(user_dict, migrate_users_to_admin_group)
        ...
        def _collect_user_groups(self, user_dict: dict, migrate_users_to_admin_group: bool) -> list:
            """Collect the groups for a user."""    
            user_groups = [self._groups[group_id] for group_id in user_dict.get("group_ids", []) if group_id in self._groups]
            if not user_dict["system_generated"] and migrate_users_to_admin_group:
                user_groups.append(self._groups[GROUP_ID_ADMIN])
            return user_groups

        def _load_credentials(self, data: dict) -> None:
        ...
        def _load_refresh_tokens(self, data: dict) -> None:
            """Load refresh tokens from data."""
            for rt_dict in data["refresh_tokens"]:
                # Filter out the old keys that don't have jwt_key (pre-0.76)
                if "jwt_key" not in rt_dict:
                    continue
                token = self._parse_refresh_token(rt_dict)
                self._users[rt_dict["user_id"]].refresh_tokens[token.id] = token

        def _parse_refresh_token(self, rt_dict: dict) -> models.RefreshToken:
            """Parse refresh token details from the refresh token dictionary."""
            created_at = dt_util.parse_datetime(rt_dict["created_at"])
            last_used_at = dt_util.parse_datetime(rt_dict.get("last_used_at"))
            token_type = rt_dict.get("token_type", models.TOKEN_TYPE_SYSTEM if rt_dict["client_id"] is None else models.TOKEN_TYPE_NORMAL)
            ...
            return models.RefreshToken(
                ...
                credential=self._credentials.get(rt_dict.get("credential_id"))
            )
