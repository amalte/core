...
class AlexaThermostatController(AlexaCapability):
    ...
    def _get_thermostat_mode(self) -> Any:
        """Return thermostat mode property."""
...
class AlexaModeController(AlexaCapability):
    ...
    def get_property(self, name: str) -> Any:
        ...
        handlers = {
            f"{fan.DOMAIN}.{fan.ATTR_DIRECTION}": self._fan_direction,
            f"{fan.DOMAIN}.{fan.ATTR_PRESET_MODE}": self._fan_preset_mode,
            f"{humidifier.DOMAIN}.{humidifier.ATTR_MODE}": self._humidifier_mode,
            f"{remote.DOMAIN}.{remote.ATTR_ACTIVITY}": self._remote_activity,
        }

        handler = handlers.get(self.instance or "")
        if handler:
            return handler()
    ...
    def _fan_direction(self) -> Any:
        """Return fan direction property."""
    ...
    def _fan_preset_mode(self) -> Any:
        """Return fan preset mode property."""
    ...
    def _humidifier_mode(self) -> Any:
        """Return humidifier mode property."""
    ...
    def _remote_activity(self) -> Any:
        """Return remote activity property."""
    ...
    def capability_resources(self) -> dict[str, list[dict[str, Any]]]:
        ...
        if self.instance == f"{humidifier.DOMAIN}.{humidifier.ATTR_MODE}":
            return self._humidifier_modes_capability_resources()

        # Water heater operation modes
        if self.instance == f"{water_heater.DOMAIN}.{water_heater.ATTR_OPERATION_MODE}":
            return self._water_heater_capability_resources()

        # Remote Resource
        if self.instance == f"{remote.DOMAIN}.{remote.ATTR_ACTIVITY}":
            return self._remote_capability_resources()
        ...
        if self.instance == f"{valve.DOMAIN}.state":
            return self._valve_position_capability_resources()

        return {}

    def _humidifier_modes_capability_resources(self) -> dict[str, list[dict[str, Any]]]:
        """Return humidifier modes capabilityResources object."""
    ...
    def _water_heater_capability_resources(self) -> dict[str, list[dict[str, Any]]]:
        """Return water heater operation modes capabilityResources object."""
        self._resource = AlexaModeResource([AlexaGlobalCatalog.SETTING_MODE], False)
        operation_modes = (
            self.entity.attributes.get(water_heater.ATTR_OPERATION_LIST) or []
        )
    ...
    def _remote_capability_resources(self) -> dict[str, list[dict[str, Any]]]:
        """Return remote capabilityResources object."""
    ...
    def _valve_position_capability_resources(self) -> dict[str, list[dict[str, Any]]]:
        """Return valve position capabilityResources object."""
        supported_features = self.entity.attributes.get(ATTR_SUPPORTED_FEATURES, 0)
        self._resource = AlexaModeResource(
...
class AlexaRangeController(AlexaCapability):
    ...
    def get_property(self, name: str) -> Any:
        ...
        handlers = {
            # Cover Position
            f"{cover.DOMAIN}.{cover.ATTR_POSITION}": lambda: self.entity.attributes.get(
                cover.ATTR_CURRENT_POSITION
            ),
            # Cover Tilt
            f"{cover.DOMAIN}.tilt": lambda: self.entity.attributes.get(
            ...
            f"{humidifier.DOMAIN}.{humidifier.ATTR_HUMIDITY}": lambda: self.entity.attributes.get(
                humidifier.ATTR_HUMIDITY, 0
            ),
            # Input Number Value
            f"{input_number.DOMAIN}.{input_number.ATTR_VALUE}": lambda: float(
                self.entity.state
            ),
            # Number Value
            f"{number.DOMAIN}.{number.ATTR_VALUE}": lambda: float(self.entity.state),
        }

        handler: Any = handlers.get(self.instance or "")
        if handler:
            return handler()
    ...
    def capability_resources(self) -> dict[str, list[dict[str, Any]]]:
        ...
        if self.instance == f"{vacuum.DOMAIN}.{vacuum.ATTR_FAN_SPEED}":
            return self._vacuum_fan_speed_resources()
    ...
    def _vacuum_fan_speed_resources(self) -> dict[str, list[dict[str, Any]]]:
        """Return vacuum fan speed capabilityResources object."""
