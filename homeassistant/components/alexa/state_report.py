...
async def async_enable_proactive_mode(
    ...
    def _is_valid_event(data: EventStateChangedData) -> bool:
        """Check if the event is valid and should be processed."""
        new_state = data.get("new_state")
        return (
            hass.is_running
            and new_state
            and new_state.domain in ENTITY_ADAPTERS
            and smart_home_config.should_expose(data["entity_id"])
        )

    @callback
    def _async_entity_state_filter(data: EventStateChangedData) -> bool:
        """Filter valid state change events for Alexa."""
        return _is_valid_event(data)

    def _get_reporting_flags(alexa_entity: AlexaEntity) -> tuple[bool, bool]:
        """Determine if the entity should report proactively or trigger doorbell."""
        should_report = any(
            i.properties_proactively_reported() for i in alexa_entity.interfaces()
        )
        should_doorbell = any(
            i.name() == "Alexa.DoorbellEventSource" for i in alexa_entity.interfaces()
        )
        return should_report, should_doorbell

    async def _handle_doorbell_event(
        new_state: State, old_state: State | None, alexa_entity: AlexaEntity
    ) -> None:
    ...
    async def _send_change_report_if_needed(
        alexa_entity: AlexaEntity, new_state: State
    ) -> None:
        """Send a change report if there was a significant change."""
    ...
    async def _async_entity_state_listener(
        ...
        """Handle state change events for Alexa."""
        data = event_.data
        new_state = data["new_state"]
        if not new_state:
            return

        alexa_entity = ENTITY_ADAPTERS[new_state.domain](
            hass, smart_home_config, new_state
        )
        should_report, should_doorbell = _get_reporting_flags(alexa_entity)

        if should_doorbell:
            await _handle_doorbell_event(new_state, data["old_state"], alexa_entity)
            return

        if should_report:
            await _send_change_report_if_needed(alexa_entity, new_state)
